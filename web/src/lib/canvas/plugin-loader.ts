...
// Whitelist of plugin origins. Add your official plugin host here for production.
...
function assertAllowedOrigin(url: string) {
    let parsed: URL;
    try {
        parsed = new URL(url);
    } catch {
        throw new Error("插件 URL 格式不正确");
    }
    if (!ALLOWED_PLUGIN_ORIGINS.includes(parsed.origin)) {
        throw new Error(`不允许的插件来源：${parsed.origin}`);
    }
}
