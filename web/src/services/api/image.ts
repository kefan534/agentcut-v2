...
import { isRemoteModel, remoteImageGeneration, remoteImageEdit, remoteImageQuestion } from "./remote-gateway";
...
function resolveGeminiImageConfig(config: AiConfig) {
    ...
    const imageSize = supportsGeminiImageSize(config.model) ? resolveGeminiImageSize(config.quality, dimensions, config.resolution) : undefined;
...
function resolveGeminiImageSize(quality: string, dimensions: { width: number; height: number } | null, resolution: string | undefined) {
    const tier = normalizeResolutionTier(resolution);
    if (tier) return tier;
...
function readAxiosError(error: unknown, fallback: string) {
    ...
    if (axios.isAxiosError(error)) {
        ...
        // 没有响应时（网络错误、超时、CORS 阻断等），保留 axios 原始提示
        if (!error.response) {
...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    ...
    if (isRemoteModel(config, selectedModel)) {
        try {
            const responseData = await remoteImageGeneration(config, prompt, options);
            return parseImagePayload(responseData as ImageApiResponse);
        } catch (error) {
    ...
    if (script) {
        ...
        try {
            const result = await runModelPlugin({
                ...
                images: [],
                params: { size: requestSize, quality, count: n, resolution: config.resolution, ...(background ? { background } : {}) },
                signal: options?.signal,
            });
    ...
    const requestSize = resolveRequestSize(config.resolution, quality, config.size);
    const background = normalizeBackground(config.background);
    const isGptImage = requestConfig.model.toLowerCase().startsWith("gpt-image-");
    try {
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
    ...
    if (isRemoteModel(config, selectedModel)) {
        try {
            const responseData = await remoteImageEdit(config, prompt, references, mask, options);
            return parseImagePayload(responseData as ImageApiResponse);
        } catch (error) {
            throw new Error(readAxiosError(error, "请求失败"));
        }
...
export async function requestImageQuestion(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    ...
    if (isRemoteModel(config, selectedModel)) {
        try {
            return await remoteImageQuestion(config, messages, onDelta, options);
        } catch (error) {
            throw new Error(readAxiosError(error, "请求失败"));
        }
    }
