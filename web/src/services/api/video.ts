...
export async function pollVideoGenerationTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    ...
    if (isRemoteModel(config, task.model)) {
        try {
            const video = unwrapVideoResponse(await remoteVideoTaskStatus(config, task.id, options));
            const url = videoResultUrl(video);
            if (url) return { status: "completed", result: await videoResultFromUrl(url, options) };
            if (video.status === "completed") {
                const content = await remoteVideoTaskContent(config, task.id, options);
...
function unwrapVideoResponse(payload: Record<string, unknown>) {
    return unwrapEnvelope(payload as ApiVideoResponse, "接口没有返回视频任务");
}
